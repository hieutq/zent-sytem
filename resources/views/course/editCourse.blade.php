...
<div class="portlet light bordered" id="form_wizard_1">
   <div class="portlet-title">
      <div class="caption">
         <span class="caption-subject font-red bold uppercase"><i class="fa fa-plus-circle" aria-hidden="true"></i> SỬA THÔNG TIN KHÓA HỌC  </span>
      </div>
   </div>
   ...
   @if(count($errors))
    ...
    @endif
      <form  action="{{route('courses.update', $Course->id)}}" id="frmCreateCourse" name="frmCreateCourse" method="POST" enctype="multipart/form-data">
         {{ csrf_field() }}
         {{ method_field('PUT') }}
                  ...
                  <div class="tab-pane active">
                     <div class="form-body col-xs-12 col-sm-12 col-md-8 col-md-offset-2 col-lg-10 col-lg-offset-1">
                        ...
                        <div class="form-group form-md-line-input form-md-floating-label {{ $errors->has('short_name') ? 'has-error' : '' }}">
                           <input type="text" class="form-control" id="short_name" name="short_name" value="{{ $Course->short_name}}">
                           <label for="code">Tên viết tắt <span class="requireds"> (*)</span></label>
                        </div>
                        <p class="font-red-mint">{{ $errors->first('short_name') }}</p>

                        <div class="form-group form-md-line-input form-md-floating-label ">
                           <input type="text" class="form-control" id="slogan" name="slogan" value="{{ $Course->slogan}}">
                           <label for="code">Khẩu hiệu</label>
                        </div>

                        <div class="form-group form-md-line-input form-md-floating-label">
                           <input class="form-control" id="tuition" name="tuition"  type="text" value="{{ $Course->tuition}}">
                        ...
                        <div class="form-group form-md-line-input form-md-floating-label {{ $errors->has('capacity') ? 'has-error' : '' }}">
                           <input type="number" min = "1" class="form-control" id="capacity" name="capacity" value="{{ $Course->capacity}}">
                        ...
                        <div class="form-group form-md-line-input form-md-floating-label {{ $errors->has('orientation_time') ? 'has-error' : '' }}">
                           <input type="text" class="form-control" id="orientation_time" name="orientation_time"  value="{{ $Course->orientation_time}}">
                        ...
                        <div class="form-group form-md-line-input form-md-floating-label">
                           <input type="text" class="form-control" id="time_table" name="time_table" value="{{ $Course->time_table}}">
                           <label for="time_table">Lịch học</label>
                        </div>

                        <div class="form-group form-md-line-input form-md-floating-label">
                           <input type="text" class="form-control" id="class_fb_group" name="class_fb_group" value="{{ $Course->class_fb_group}}">
                        ...
                        <div class="form-group form-md-line-input form-md-floating-label {{ $errors->has('status') ? 'has-error' : '' }}" >
                           <select  id = "status" class="form-control" name="status">
                              <option value=""></option>
                              <option value="0" @if ($Course->status==0) Selected  @endif >Ẩn</option>
                              <option value="1" @if ($Course->status==1) Selected  @endif >Hiển thị</option>
                           </select>
                        ...
                        <div class="form-group">
                            <div style="margin-bottom: 15px;color: #888;">Thông tin lớp học</div>
                              <textarea class="form-control summernote" rows="3" id="class_info" name="class_info">{{ $Course->class_info}}</textarea>
                          
                        </div>

                        <div class="form-group">
                            <div style="margin-bottom: 15px;color: #888;">Thông tin lớp chi tiết</div>
                              <textarea class="form-control summernote" rows="3" id="class_desire_detail" name="class_desire_detail">{{ $Course->class_desire_detail}}</textarea>
                           
                        </div>

                        <div class="form-group ">
                           <div style="margin-bottom: 15px;color: #888;">Đối tượng học viên</div>
                              <textarea class="form-control summernote" rows="3" id="student_object" name="student_object">{{ $Course->student_object}}</textarea>
                           
                        </div>

                        <div class="form-group">
                           <div style="margin-bottom: 15px;color: #888;">Nội dung</div>
                              <textarea class="form-control summernote" rows="3" id="content" name="content" placeholder="Nội Dung">{{ $Course->content}}</textarea>
                           
                        </div>

                        <div class="form-group">
                           <div style="margin-bottom: 15px;color: #888;">Thông tin đăng ký</div>
                              <textarea class="form-control summernote" rows="3" id="register_info" name="register_info" placeholder="Thông Tin Đăng Ký">{{ $Course->register_info}}</textarea>
