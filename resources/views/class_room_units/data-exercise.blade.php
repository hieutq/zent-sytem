	...
	@foreach($theories as $key => $theory)
    	<tr>
            ...
            <td class="text-center"> 
            	<a href="#" class="show-content-exercise btn btn-outline btn-circle btn-sm blue" data-name = "" data-toggle="modal" data-target="#myModal">
                  ...
                  <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                      <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myModalLabel">{{$theory->name}}</h4>
                      </div>
            ...
            <td class="text-center"> 
              <input type="hidden" id="checked-{{$theory->id}}-{{$class_room_unit_id}}" value="{{$theory->checked}}">
            	...
            	@if($theory->checked)
                 ...
                 <i id="action-{{$theory->id}}-{{$class_room_unit_id}}" class="fa fa-check-circle" onclick="addTheory({{$theory->id}}, {{$class_room_unit_id}})" data-id="{{$theory->id}}" data-class_rom_unit_id="{{$class_room_unit_id}}" aria-hidden="true" style="cursor: pointer; color: #3598dc;"></i>
              @else 

                <i id="action-{{$theory->id}}-{{$class_room_unit_id}}" class="fa fa-circle-o"  onclick="addTheory({{$theory->id}}, {{$class_room_unit_id}})" aria-hidden="true" style="cursor: pointer; color: #3598dc;"></i>
