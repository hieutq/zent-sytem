                        ...
                        <div class="tab-pane @if ($step == 1) active @endif" id="tab1">
                                ...
                                <div class="form-body col-xs-12 col-sm-12 col-md-8 col-md-offset-2 col-lg-10 col-lg-offset-1">
                                   ...
                                   <div class="form-group form-md-line-input form-md-floating-label">
                                      <textarea type="textarea" rows="4" class="form-control" id="note" name="note"></textarea>
                                       ...
                                       <input type="hidden" class="form-control" id="class_room_id" name="class_room_id" value="{{$class_room_id}}">
                                   </div>       
                                </div>    
                                ...
                                <div class="form-actions text-center">
                                      <div class="col-xs-12 col-sm-12">
                                          ...
                                          <a href="javascript:;" onclick="funAddUnit();" class="btn btn-outline green button-next  btn-circle"> Thêm mới</a>
                                      </div>
                                </div>
                        </div>
                                                      ...
                                                      @foreach($theory_groups as $theory_group)
                                                        <optgroup label="{{$theory_group->name}}">
                                                            @if(!empty($theory_group->theories)) @foreach($theory_group->theories as $theory)
                                                                <option value="{{$theory->id}}" data-tokens="{{$theory->name}}">{{$theory->name }}</option>
                                                            @endforeach @endif
                                                        </optgroup>
                                                      @endforeach
  ...
  <script>
    ...
    function funAddUnit() {
      ...
      $.ajaxSetup({
        ...
        $.ajax({
              type: "POST",
              url: "{{route('classroom-unit.store')}}",
              data: {
              ...
              success: function(res)
              {
                ...
                if(!res.error) {
                    ...
                    toastr.success('Thêm thông tin bài tập thành công!');
                    setTimeout(function () {
                        window.location.href = "{{route('classroom-unit.create')}}" + "?class_room_id=" + res.data.class_room_id + '&class_room_unit_id=' + res.data.id + '&step=2' 
                    }, 1500);                   
                } else {
    ...
    function nextStep(class_room_id, class_room_unit_id) {
      setTimeout(function () {
          window.location.href = "{{route('classroom-unit.create')}}" + "?class_room_id=" + class_room_id + '&class_room_unit_id=' + class_room_unit_id + '&step=3' 
      }, 0);  
    }
