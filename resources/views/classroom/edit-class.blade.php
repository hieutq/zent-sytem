...
<div class="portlet light bordered" id="form_wizard_1">
   <div class="portlet-title">
      <div class="caption">
         <i class="fa fa-pencil-square-o" aria-hidden="true"></i> CẬP NHẬT LỚP HỌC  
      </div>
   </div>
   <div class="portlet-body form">
      <form  id="frmEditClassRoom" method="PUT" name="frmEditClassRoom" role="form"  enctype="multipart/form-data">
         {{ csrf_field() }}
         <div class="form-wizard">
                  ...
                  <div class="tab-pane active" id="tab1">
                     ...
                     <div class="form-body col-xs-12 col-sm-12 col-md-8 col-md-offset-2 col-lg-10 col-lg-offset-1">
                        <div class="form-group form-md-line-input form-md-floating-label">
                           ...
                           <input type="text" class="form-control" id="code" name="code" value="{{$detail_class->code}}">
                           <label for="code">Mã Lớp (<span style="color:red;">*</span>)</label>
                          <span class="help-block"></span>
                        </div>
                        <div class="form-group form-md-line-input form-md-floating-label">
                           <input type="text" class="form-control" id="class_name" name="class_name" value="{{$detail_class->class_name}}">
                        ...
                        <label class="lable-select" for="id_teacher">Giảng Viên</label>
                           <select class="selectpicker " id="id_teacher" name="id_teacher[]" multiple="" data-live-search="true" tabindex="-98">
                              @if(!empty($list_teacher))
                                  ...
                                  <option @if (in_array($db_user->id, $arr_teacher)) selected @endif value="{{$db_user->id}}" data-tokens="{{$db_user->name}},{{$db_user->mobile}},{{$db_user->email}}">{{$db_user->name }} </option>
                                  @endforeach
                               @endif
                        ...
                        <div class="form-group form-md-line-input form-md-floating-label">
                          <label class="lable-select" for="id_teacher">Trợ Giảng</label>
                           <select id="tutors_name_class" class="selectpicker" name="tutors_name_class[]" multiple="" data-live-search="true" tabindex="-98">
                              ...
                              <option @if (in_array($db_tutor->id, $arr_tutor)) selected @endif value="{{$db_tutor->id}}" data-tokens="{{$db_tutor->name}},{{$db_tutor->mobile}},{{$db_tutor->email}}">{{$db_tutor->name }}</option>
                              @endforeach
                              @endif
                        ...
                        <div class="form-group form-md-line-input form-md-floating-label">
                           <input class="form-control" id="orientation_time"
                              name="orientation_time" type="text" value="{{$detail_class->orientation_time}}" />
                           <label for="orientation_time" class="lb-orientation_time">Ngày Khai Giảng</label>
                    
                        </div>
                        <div class="form-group form-md-line-input form-md-floating-label">
                           <input type="text"  class="form-control" id="time_table" name="time_table" value="{{$detail_class->time_table}}">
                           <label for="time_table">Ngày Học</label>
                          
                        </div>
                        <div class="form-group form-md-line-input form-md-floating-label">
                           <input type="text" class="form-control" id="number_of_unit" name="number_of_unit" value="{{$detail_class->number_of_unit}}">
                           <label for="number_of_unit">Số Buổi Học (<span style="color:red;">*</span>)</label>
                       
                        </div>
                        <div class="form-group form-md-line-input form-md-floating-label">
                           <input type="text" class="form-control" id="tuition" name="tuition" value="{{$detail_class->tuition}}">
                           <label for="tuition">Học Phí (<span style="color:red;">*</span>)</label>
                      
                        </div>
                        <div class="form-group form-md-line-input form-md-floating-label">
                           <input type="text" class="form-control" id="max_tuition_policy" name="max_tuition_policy" value="{{$detail_class->max_tuition_policy}}">
                        ...
                        <div class="form-group form-md-line-input form-md-floating-label">
                           <select  id = "course_id" class="form-control" name="course_id">
                              ...
                              @foreach($list_course as $db)
                                <option value="{{$db->id}}" @if ($db->id == $detail_class->course_id) selected @endif>{{$db->name}}</option>
                              @endforeach
                              @else
                              <option value=""></option>
                              @endif
                        ...
                        <div class="form-group form-md-line-input form-md-floating-label">
                           <input type="text" class="form-control" id="facebook_group" name="facebook_group" value="{{$detail_class->facebook_group}}">
                           <label for="facebook_group">Facebook Lớp</label>
          
                        </div>
                        <div class="form-group form-md-line-input form-md-floating-labelr">
                           <select  id = "status" class="form-control" name="status">
                              <option value=""></option>
                              @if (!empty($statuses)) @foreach($statuses as $status)
                                <option value="{{$status->id}}" @if ($detail_class->status == $status->id) selected @endif>{{$status->name}}</option>
                              @endforeach @endif
                           </select>
                        ...
                        <div class="form-group">
                           <div class="col-sm-12">
                              <textarea id="tuition_policy" class="EditorControl" style="height: 250px; width: 100%"" placeholder="Chính Sách Giảm Học Phí" name="tuition_policy">{{$detail_class->tuition_policy}}
                              </textarea> 
                              <input type="hidden" id="class_id" name="class_id" value="{{$detail_class->id}}">                          
                           </div>
                        </div>
                     </div>
                  </div>
            ...
            <div class="form-actions text-center">
                  ...
                  <div class="col-xs-12 col-sm-12">
                     <button type="submit" id="edit-class-room" class="btn btn-outline green button-next btn-circle">Cập nhật</button>
                  </div>
            </div>
         </div>
      </form>
...
{{-- <script>
  $('#frmEditClassRoom').validate({ // initialize the plugin
            errorElement: "span",
            rules: {
              code : {
                required : true,
        ...
        submitHandler: function(form) {
          ...
          $('#frmEditClassRoom button[type="submit"]').prop('disabled',true);
          ...
          $.ajax({
              type:'PUT',
              url: "{{URL::asset('')}}classroom/" + $("#class_id").val(),
              data: {
                code : $("#code").val(),
              ...
              success:function(data){
                  ...
                  if(!data.error) {
                      ...
                      $('#frmEditClassRoom button[type="submit"]').prop('disabled',false);
                  }
                },
...
<script>
    ...
    $('#frmEditClassRoom').on('submit',function(e){
